...
const placeholderImage = Image(
  image: AssetImage('assets/images/photo_unavailable_placeholder_basic.jpg'),
);
...
class _GuideState extends State<Guide> {
  static final List<GuideContent> localContents = <GuideContent>[
    GuideContent(
      ...
      content:
          "To reduce the production of such waste, it’s important to practice waste management at home. You can make small changes in your home which can lead to a big impact. Take a look at these simple ways in which waste can be efficiently managed at home:\n"
          "\n"
    ...
    GuideContent(
        ...
        content:
            "There are 6 types of waste that can be classified into two categories: recyclable and non-recyclable.\n\n"
...
class GuideArticle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final int id =
        int.tryParse(GoRouterState.of(context).pathParameters['id'] ?? '') ??
            -1;
    final GuideContent guideContent =
        (0 <= id && id < _GuideState.contents.length)
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: ListView(
          ...
          children: <Widget>[
            Text(
              guideContent.title ?? 'Guide not found',
            ...
            guideContent.imagePath != null
                ? SizedBox(
                    height: 170,
                    child: Image(
                      image: AssetImage(guideContent.imagePath!),
                      errorBuilder: (context, error, stackTrace) =>
                          placeholderImage,
                      fit: BoxFit.contain,
                    ))
            ...
            Padding(
              ...
              child: Text(
                guideContent.content ?? 'Guide with id $id not found',
